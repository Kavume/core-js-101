...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const brackets = ['[', ']', '(', ')'];
  const [openSquare, closeSquare, openRound, closeRound] = brackets;
  if (isStartIncluded === true) this.bracketOpen = openSquare;
  if (isStartIncluded === false) this.bracketOpen = openRound;
  if (isEndIncluded === true) this.bracketClose = closeSquare;
  if (isEndIncluded === false) this.bracketClose = closeRound;
  let min = 0;
  let max = 0;
  if (a > b) {
    min = b;
  ...
  return `${this.bracketOpen}${min}, ${max}${this.bracketClose}`;
}
